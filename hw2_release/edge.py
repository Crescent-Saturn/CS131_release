...
def conv(image, kernel):
    ...
    h, w = Hk // 2, Wk // 2
    temp = 0  # np.zeros((padded_img.shape[0], padded_img.shape[1]))
    for i in range(h, Hi + h):
        for j in range(w, Wi + w):
            temp = np.sum(
                padded[i - h:i + h + 1, j - w:j + w + 1] * kernel)
...
def hough_transform(img):
    ...
    for i in range(len(xs)):
        ...
        for theta_in in range(num_thetas):
            rho = int(x * cos_t[theta_in] + y * sin_t[theta_in]) + diag_len
            # print(rho)
            accumulator[rho, theta_in] += 1
    # END YOUR CODE
